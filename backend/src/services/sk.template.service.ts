...
const getPembimbingAktifData = async () => {
    return {
            teknik_informatika: {
                tetap: [
                    {
...
export const generateSKPreviewService = async (data: {
    ...
    jenis_sk: string;
    semester: number;
    tahun_akademik: string | number | null;
    tanggal: string;
    NIP_dekan: string;
    nama_dekan: string;
}) => {
    ...
    if (typeof data.tahun_akademik === "number") {
        data.tahun_akademik = data.tahun_akademik.toString() + "/" + (data.tahun_akademik + 1).toString()
    }
