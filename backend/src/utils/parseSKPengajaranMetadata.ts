...
export const parseSKPengajaranMetadata = async (filePath: string): Promise<ParsedSKMetadata> => {
  ...
  const tahunAkademikMatch = text.match(/SEMESTER\s+[IVX]+\s*[–/-]\s*(\d{4})\s*[/–-]\s*(\d{4})/i)
