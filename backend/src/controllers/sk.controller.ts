...
// Modified preview function - now generates and saves the file
export const generatePreviewSK = async (req: Request, res: Response) => {
  try {
    const data = req.body

    // For temporary preview without saving to database
    if (!data.no_sk) {
      ...
      // Generate and save for existing draft
      await generateAndSaveDraftSKService(data.no_sk)

      // Return the saved file
      const filePath = await getDownloadPathService(data.no_sk)
...
// New endpoint for draft SK preview using saved file
export const previewDraftSK = async (req: Request, res: Response) => {
  try {
    const { no_sk } = req.params
    const originalNoSK = no_sk.replace(/_/g, "/")

    // Generate and save the file if it doesn't exist
    await generateAndSaveDraftSKService(originalNoSK)
    ...
    console.error("Error getting draft SK preview:", err)
    res.status(500).json({ message: "Gagal preview draft SK" })
  }
}
